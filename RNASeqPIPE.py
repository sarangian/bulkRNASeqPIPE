...
########################################################################################################################
		...
		cmd_run_trinity_pe = "[ -d  {assembled_transcript_folder} ] || mkdir -p {assembled_transcript_folder}; " \
						  "cd {clean_read_folder}; " \
							 ...
							 "--CPU 4 "\
			.format(assembled_transcript_folder=assembled_transcript_folder,
					clean_read_folder=clean_read_folder,trinity_pe=cmd_trinity_pe_input)
		...
		cmd_run_trinity_se = "[ -d  {assembled_transcript_folder} ] || mkdir -p {assembled_transcript_folder}; " \
							 ...
							 "--CPU 4 " \
			.format(assembled_transcript_folder=assembled_transcript_folder,
					clean_read_folder=clean_read_folder, trinity_se=cmd_trinity_se_input)
...
########################################################################################################################
		...
		cmd_run_rnaspades_pe = "[ -d  {assembled_transcript_folder} ] || mkdir -p {assembled_transcript_folder}; " \
							   ...
							   "-o {assembled_transcript_folder} " \
							   "-t 1 " \
			.format(clean_read_folder=clean_read_folder,
...
class salmonQuant(luigi.Task):
	...
	def requires(self):
		return [denovoTransAssemble(projectName=self.projectName,
							   domain=self.domain,
							   adapter=self.adapter,
							   readType=self.readType,
...
class makeTx2Gene(luigi.Task):
	...
	def run(self):
		...
		cmd_tx2gene_from_gtf_euk = "cd {transcriptomeFolder}; tx2gene.R " \
							   "-a gtf " \
		...
		cmd_tx2gene_from_gtf_pro = "[ -d  {transcriptomeFolder} ] || mkdir -p {transcriptomeFolder}; cd {transcriptomeFolder};" \
								  "cat {transcriptomeFolder}{transcriptName}.gtf |{tx2g_cmd1} | {tx2g_cmd2} > tx2gene.csv" \
								  .format(transcriptomeFolder=os.path.join(GlobalParameter().basefolder, "raw_data", "transcriptome",self.transcriptName + "/"),
										  transcriptName=self.transcriptName,tx2g_cmd1=tx2g_cmd1, tx2g_cmd2=tx2g_cmd2)





		if (self.annotFileType == "GTF") and (self.domain == "eukaryote"):

			print ("****** NOW RUNNING COMMAND ******: " + cmd_copy_gtf)
			print (run_cmd(cmd_copy_gtf))

			print ("****** NOW RUNNING COMMAND ******: " + cmd_tx2gene_from_gtf_euk)
			print (run_cmd(cmd_tx2gene_from_gtf_euk))



		if (self.annotFileType == "GTF") and (self.domain == "prokaryote"):

			print ("****** NOW RUNNING COMMAND ******: " + cmd_copy_gtf)
			print (run_cmd(cmd_copy_gtf))

			print ("****** NOW RUNNING COMMAND ******: " + cmd_tx2gene_from_gtf_pro)
			print (run_cmd(cmd_tx2gene_from_gtf_pro))



		if (self.annotFileType == "GFF") and (self.domain == "eukaryote"):
		...
		if (self.annotFileType == "GFF") and (self.domain == "prokaryote"):
...
class quantifyTranscripts(luigi.Task, TimeTask):
	...
	def output(self):
		transcriptFolder = os.path.join(GlobalParameter().basefolder, "raw_data", "transcriptome",self.transcriptName + "/")
		return {'out': luigi.LocalTarget(transcriptFolder +"/"  + "tx2gene.csv")}
