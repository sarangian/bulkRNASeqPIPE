...
class indexDAT(luigi.Task):
	...
	def requires(self):
		return [dnTransAssemble(pre_process_reads=self.pre_process_reads,
...
class denovoQuant(luigi.Task):
	...
	def requires(self):
		return [indexDAT(pre_process_reads=self.pre_process_reads,
						 rnaseq_assembler=self.rnaseq_assembler)]

	def output(self):
		salmon_quant_folder = os.path.join(os.getcwd(), GlobalParameter().project_name, "deNovoDEA", "ReadQuant",
...
class clusterDAT(luigi.Task):
	...
	def requires(self):
		...
		return [
				[quantifyDAT(pre_process_reads=self.pre_process_reads,
							rnaseq_assembler=self.rnaseq_assembler)]
				]


	def output(self):
		corset_folder = os.path.join(os.getcwd(), GlobalParameter().project_name, "deNovoDEA",
									 "ReadQuant",
...
class denovoDEA(luigi.Task):
	...
	factor_of_intrest = luigi.Parameter(default="conditions",description="factor of intrest column of the target file (string [=conditions]). ")
	...
	def requires(self):
		return [clusterDAT(rnaseq_assembler=self.rnaseq_assembler,
						   pre_process_reads=self.pre_process_reads)]
	...
	def output(self):
		...
		resultFolder = os.path.join(os.getcwd(), GlobalParameter().project_name, "deNovoDEA",
									"DEAnalysis",
